    ...
    <script>
        $(function () {
            ...
            $("#captchaImage").click(function () {
                $("#captchaImage").prop("src","${path}/admin/getCode?d="+new Date().getTime());
            });

            $.extend($.validator.messages, {
                required: "<span style='color: red'>必填字段!</span>",
                minlength: $.validator.format("<span style='color: red'>请输入不小于 {0} 个字段!</span>"),
            });

            let status;
            //表单
            $("#loginButtonId").click(function () {
                let isOK = $("#loginForm").valid();
                if(isOK){
                    $.ajax({
                    ...
                    <div class="form-bottom" style="width: 450px">
                        <form role="form" method="post" class="login-form" id="loginForm">
                            ...
                            <div class="form-group">
                                ...
                                <input minlength="5" type="text" name="username" placeholder="请输入用户名..."
                                       class="form-username form-control" required id="form-username">
                            </div>
                            <div class="form-group">
                                <label class="sr-only" for="form-password">Password</label>
                                <input minlength="5" type="password" name="password" placeholder="请输入密码..."
                                       class="form-password form-control" required id="form-password">
                            </div>
                            <div class="form-group">
                                <%--<label class="sr-only" for="form-code">Code</label>--%>
                                <img id="captchaImage" style="height: 48px;width: 101px" class="captchaImage" src="${path}/admin/getCode">
                                <input minlength="4" style="width: 289px;height: 50px;border:3px solid #ddd;border-radius: 4px;"
                                       required type="test" name="core" id="form-code">
                            </div>
